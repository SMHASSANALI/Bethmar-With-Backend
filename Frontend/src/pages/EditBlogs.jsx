...
const EditBlogs = () => {
    ...
    const handleSave = async () => {
        const updatedBlog = { title, description, image, category, content, status };
        try {
            await putDataFromAPI(`blog-posting/update-blog-posting/${state._id}`, updatedBlog);
        } catch (error) {
        ...
        <div className='bg-[#f9f9f9]'>
            <div className="max-w-6xl mx-auto px-4 py-8 font-sans bg-[#f6f8f7]">
                ...
                <div className="flex items-center gap-2 mb-4">
                    {editMode.title ? (
                        ...
                        <>
                            ...
                            <FiEdit2 className="text-gray-500 cursor-pointer" onClick={() => setEditMode({ ...editMode, title: true })} />
                        </>
                    )}
                </div>
                ...
                <div className="flex items-center gap-2 mb-6">
                    {editMode.description ? (
                        ...
                        <>
                            ...
                            <FiEdit2 className="text-gray-500 cursor-pointer" onClick={() => setEditMode({ ...editMode, description: true })} />
                        </>
                    )}
                </div>
                ...
                <div className="flex items-center gap-2 mb-6">
                    {editMode.category ? (
                        ...
                        <>
                            ...
                            <FiEdit2 className="text-gray-500 cursor-pointer" onClick={() => setEditMode({ ...editMode, category: true })} />
                        </>
                    )}
                </div>
                ...
                <div className="text-sm text-gray-500 flex flex-wrap gap-4 pt-4 border-t border-gray-200 mb-6">
                    <p>Written by: <span className="text-blue-600 font-medium">BethMar</span></p>
                    <p>Category: <span className="text-blue-600 font-medium">{category}</span></p>
                    <p>
                        Published: {new Date(state.createdAt).toLocaleDateString()} |
                        <span className="text-gray-700 ml-1">Updated:</span> {new Date(state.updatedAt).toLocaleDateString()}
                    </p>
                </div>
                ...
                <div className="mb-8">
                    <div className="relative w-full max-w-md mx-auto">
                        <img src={image} alt="Blog visual" className="rounded-lg shadow-md w-full" />
                        <FiEdit2
                            className="absolute top-2 right-2 text-white bg-black/50 p-1 rounded cursor-pointer"
                            onClick={() => setEditMode({ ...editMode, image: !editMode.image })}
                        />
                    </div>
                    {editMode.image && (
                        <div className="mt-3">
                ...
                <div className="mt-10">
                    <div className="flex justify-between items-center mb-2">
                        ...
                        <FiEdit2
                            className="text-gray-500 cursor-pointer"
                            onClick={() => setEditMode({ ...editMode, content: !editMode.content })}
                        />
                    </div>
                    {editMode.content ? (
                        <ReactQuill theme="snow" value={content} onChange={setContent} />
                    ) : (
                        <div className="prose prose-lg prose-blue max-w-none" dangerouslySetInnerHTML={{ __html: content }} />
                    )}
                </div>
