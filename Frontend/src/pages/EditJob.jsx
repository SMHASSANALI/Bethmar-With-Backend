    ...
    return (
        <div className="p-6 min-h-[calc(100vh-7rem)]">
                ...
                <table className="min-w-full bg-white border border-gray-200">
                    ...
                    <tbody>
                        {jobData.map((job, index) => (
                            <tr onClick={() => onClick(job)} key={index} className="text-sm text-gray-700 hover:bg-gray-50 cursor-pointer">
                                ...
                                <td className="py-3 px-4 border">
                                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${job.status === "Published" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                                        {job.status}
                                    </span>
                                </td>
                            </tr>
            ...
            <div className="md:hidden space-y-4">
                {jobData.map((job, index) => (
                    <div onClick={() => onClick(job)} key={index} className="border p-4 rounded shadow-sm bg-white">
                        <div className="flex gap-4">
                            <div className="flex flex-col justify-between">
                                <h2 className="text-lg font-semibold">{job.jobTitle}</h2>
                                <p className="text-sm text-gray-600">{job.jobDescription}</p>
                                <p className="text-sm"><strong>Category:</strong> {job.jobType}</p>
                                <span className={`mt-1 inline-block px-2 py-1 rounded-full w-fit text-xs font-medium ${job.status === "Published" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                                    {job.status}
                                </span>
                            </div>
                        </div>
