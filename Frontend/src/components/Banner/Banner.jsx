...
const Banner = ({ image, description }) => {
  const divStyle = {
    backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.1), rgba(0, 0, 0, 0.2)), url(${image})`,
  ...
  return (
    <section
      style={divStyle}
      className='flex items-center h-[35vh] md:h-[40vh] lg:h-[70vh] w-full'
    >
      <div
        id='Banner'
        className='max-w-[1500px] md:ml-auto flex-row px-4 sm:w-full md:w-6/12'
      >
        <div className='p-2 md:py-8 md:px-12 bg-primary bg-opacity-60 backdrop-blur-sm rounded-lg relative md:top-0 top-[-50px] overflow-hidden'>
          ...
          >
            {description && description[currentSlide]}
          </motion.div>
        </div>
      </div>
